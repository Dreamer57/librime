...
bool OftenCharsetFilterTranslation::FilterCandidate(an<Candidate> cand) {
  return OftenCharsetFilter::FilterText(cand->text(),
                                           charset_,
                                           is_emoji_enabled_);
}

void OftenCharsetFilterTranslation::ParseParameters(
    const string& charset) {
  vector<string> parameters;
  boost::split(parameters, charset, boost::is_any_of("+"));
  if (!parameters.empty()) {
    charset_ = parameters[0];
    is_emoji_enabled_ = std::find(parameters.begin(),
                                  parameters.end(),
                                  "emoji") != parameters.end();
  }
}
...
bool OftenCharsetFilter::FilterText(const string& text,
  ...
  if (charset.empty()) {
    return CharsetFilter::FilterText(text);
  }
...
an<Translation> OftenCharsetFilter::Apply(
  ...
  if (engine_->context()->get_option(name_space_)) {
    return New<OftenCharsetFilterTranslation>(translation, name_space_);
  }
  return translation;
}
