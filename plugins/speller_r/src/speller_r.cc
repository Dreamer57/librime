...
SpellerR::SpellerR(const Ticket& ticket) : Processor(ticket),
                                         alphabet_(kRimeAlphabet) {
  if (Config* config = engine_->schema()->config()) {
    config->GetString("speller_r/alphabet", &alphabet_);
    config->GetString("speller_r/delimiter", &delimiters_);
    config->GetString("speller_r/initials", &initials_);
    config->GetString("speller_r/finals", &finals_);
    config->GetInt("speller_r/max_code_length", &max_code_length_);
    config->GetBool("speller_r/auto_select", &auto_select_);
    config->GetBool("speller_r/use_space", &use_space_);

    config->GetBool("speller_r/auto_select_previous", &auto_select_previous_);
    string pattern;
    if (config->GetString("speller_r/auto_select_pattern", &pattern)) {
      auto_select_pattern_ = pattern;
    }
    string auto_clear;
    if (config->GetString("speller_r/auto_clear", &auto_clear)) {
...
ProcessResult SpellerR::ProcessKeyEvent(const KeyEvent& key_event) {
  ...
  if (AutoSelectByRegex(ctx, (char)ch)) {
...
bool SpellerR::AutoSelectAtMaxCodeLength(Context* ctx) {
...
bool SpellerR::AutoSelectUniqueCandidate(Context* ctx) {
  if (!auto_select_)
    return false;
  if (!ctx->HasMenu())
    return false;
...
bool SpellerR::AutoSelectPreviousMatch(Context* ctx,
                                      Segment* previous_segment) {
  if (!auto_select_)
    return false;
...
bool SpellerR::AutoClear(Context* ctx) {
...
bool SpellerR::FindEarlierMatch(Context* ctx, size_t start, size_t end) {
  if (end <= start + 1)
    return false;
  string input = ctx->input();
...
bool SpellerR::AutoSelectByRegex(Context* ctx,
  ...
  if (!ctx->HasMenu())
    return false;
  const Segment& seg(ctx->composition().back());
  const string& input(ctx->input() + input_char);
  auto cand = seg.GetSelectedCandidate();
  bool matches_input_pattern = false;
  matches_input_pattern = boost::regex_match(input, auto_select_pattern_);
  string code(input.substr(cand->start(), cand->end()));
  if (matches_input_pattern &&
      is_auto_selectable(cand, code, delimiters_)) {
    ctx->ConfirmCurrentSelection();
    return true;
  }
  return false;
